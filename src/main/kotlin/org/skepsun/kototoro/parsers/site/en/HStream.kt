...
internal class HStream(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.HSTREAM, pageSize = 24) {

    override val configKeyDomain = ConfigKey.Domain("hstream.moe")
    ...
    override suspend fun getDetails(manga: Content): Content {
        ...
        val title = doc.selectFirst("meta[property=og:title]")?.attr("content")
            ?.removeSuffix(" - HStream")?.removeSuffix(" - hstream.moe")?.trim()
            ?: doc.selectFirst(".title, h1, .hentai-title")?.text()?.trim()
            ?: manga.title

        val description = doc.selectFirst("meta[property=og:description]")?.attr("content")
            ?: doc.selectFirst(".description, .synopsis, .entry-content")?.text()?.trim()
    ...
    override suspend fun getPages(chapter: ContentChapter): List<ContentPage> {
        ...
        if (serverButtons.isNotEmpty()) {
            return serverButtons.mapIndexed { index, btn ->
                val videoUrl = btn.attr("data-video").ifBlank { btn.attr("data-url").ifBlank { btn.attr("data-src") } }
                ContentPage(
    ...
    private fun parseList(doc: Document): List<Content> {
        ...
        val cards = doc.select("a[href*=/hentai/]").filter { el ->
        ...
        for (link in cards) {
            ...
            val title = link.selectFirst("img[alt]")?.attr("alt")?.trim()
                ?: link.selectFirst(".title, .name, .hentai-title")?.text()?.trim()
