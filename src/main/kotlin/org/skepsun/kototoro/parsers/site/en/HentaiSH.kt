...
@ContentSourceParser("HENTAISH", "Hentai.SH", "en", type = ContentType.HENTAI_VIDEO)
internal class HentaiSH(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.HENTAISH, pageSize = 24) {

    override val configKeyDomain = ConfigKey.Domain("hentai.sh")
    ...
    override suspend fun getDetails(manga: Content): Content {
        ...
        val title = doc.selectFirst("meta[property=og:title]")?.attr("content")
            ?.removeSuffix(" - Hentai.SH")?.trim()
            ?: doc.selectFirst(".video-detail h1, .video-title, .entry-title")?.text()?.trim()
            ?: manga.title

        val description = doc.selectFirst("meta[property=og:description]")?.attr("content")
            ?: doc.selectFirst(".video-description, .desc, .entry-content")?.text()?.trim()

        val cover = doc.selectFirst("meta[property=og:image]")?.attr("content")?.toAbsoluteUrlOrNull(domain)
            ?: doc.selectFirst(".video-thumb img, .thumb img, video[poster]")?.let {
                val raw = it.attr("poster").ifBlank { it.attr("data-src").ifBlank { it.attr("src") } }
                raw.toAbsoluteUrlOrNull(domain)
            }

        val tags = doc.select("a[href*=/tag/], a[href*=/genre/], a[href*=/category/]").mapNotNull { a ->
            val text = a.text().trim()
            val key = a.attr("href").substringAfterLast("/").trim()
            if (text.isNotBlank() && key.isNotBlank()) ContentTag(text, key, source) else null
        }.toSet()

        val episodeLinks = doc.select("a[href*=/video/]").filter {
    ...
    private fun parseList(doc: Document): List<Content> {
        ...
        val cards = doc.select("a[href*=/video/]").filter { el ->
            el.selectFirst("img[alt]") != null || el.selectFirst(".video-card, .title, .name") != null
        }
        for (link in cards) {
            ...
            val title = link.selectFirst("img[alt]")?.attr("alt")?.trim()
                ?: link.selectFirst(".title, .name, .video-card-title")?.text()?.trim()
