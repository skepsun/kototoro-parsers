...
@ContentSourceParser("NOVELCOOL", "NovelCool", "en", type = ContentType.NOVEL)
internal class NovelCool(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.valueOf("NOVELCOOL"), pageSize = 50) {

    override val configKeyDomain = ConfigKey.Domain("www.novelcool.com")
    ...
    private fun buildListUrl(page: Int, order: SortOrder, filter: ContentListFilter): String {
        ...
        if (q.isNotEmpty()) {
            return "https://$domain/search?keyword=$q&page=$page"
        }
        val sort = when (order) {
        ...
        val tagParam = if (tag.isNotEmpty()) "&category=$tag" else ""
        return "https://$domain/novel/list?sort=$sort&page=$page$tagParam"
    }

    private fun parseList(doc: Document): List<Content> {
        val items = ArrayList<Content>()
        val seen = LinkedHashSet<String>()
        for (el in doc.select(".book-info, .novel-item, .list-item, .book-detail, article")) {
            ...
            val title = el.selectFirst("h2, h3, .title, .book-title, .novel-title")?.text()?.trim()
    ...
    private fun parseChapters(doc: Document): List<ContentChapter> {
        ...
        for (a in doc.select(".chapter-list a, .chapters a, .chapter-item a, a[href*=chapter]")) {
