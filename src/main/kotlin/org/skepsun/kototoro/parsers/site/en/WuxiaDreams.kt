...
@ContentSourceParser("WUXIADREAMS", "WuxiaDreams", "en", type = ContentType.NOVEL)
internal class WuxiaDreams(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.WUXIADREAMS, pageSize = 50) {

    override val configKeyDomain = ConfigKey.Domain("wuxiadreams.com")
    ...
    private fun parseList(doc: Document): List<Content> {
        ...
        val items = doc.select("article.novel, .novel-item, .post")
        for (item in items) {
            val link = item.selectFirst("h2 a, .novel-title a, .entry-title a") ?: continue
    ...
    override suspend fun getDetails(manga: Content): Content {
        ...
        val descEl = doc.selectFirst(".novel-summary, .entry-content .summary, .description")
        ...
        val chapterItems = doc.select(".chapter-list li, .chapters a, .eplister a")
