...
@ContentSourceParser("ANIVIBE", "AniVibe", "en", type = ContentType.VIDEO)
internal class AniVibe(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.ANIVIBE, pageSize = 24) {

    override val configKeyDomain = ConfigKey.Domain("anivibe.to")
    ...
    private fun buildListUrl(page: Int, order: SortOrder, filter: ContentListFilter): String {
        ...
        if (q.isNotEmpty()) {
            ...
            return "https://$domain/search?q=$q&page=$page$genreParam"
        }
