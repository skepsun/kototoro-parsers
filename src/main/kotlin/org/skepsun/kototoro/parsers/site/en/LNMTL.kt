...
@ContentSourceParser("LNMTL", "LNMTL", "en", type = ContentType.NOVEL)
internal class LNMTL(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.LNMTL, pageSize = 50) {

    override val configKeyDomain = ConfigKey.Domain("lnmtl.com")
    ...
    override suspend fun getListPage(page: Int, order: SortOrder, filter: ContentListFilter): List<Content> {
        ...
        val url = if (query.isNotBlank()) {
            "https://$domain/novel?q=${query.urlEncoded()}&page=$page"
        } else {
            "https://$domain/novel?page=$page"
        }
    ...
    private fun parseList(doc: Document): List<Content> {
        ...
        val items = doc.select(".novel-item, .media")
        for (item in items) {
            val link = item.selectFirst("a") ?: continue
            val href = link.attr("href")
            val titleEl = item.selectFirst(".novel-title, .media-heading")
    ...
    override suspend fun getDetails(manga: Content): Content {
        ...
        val title = doc.selectFirst("h1.novel-title, h1")?.text()?.trim() ?: manga.title

        val coverEl = doc.selectFirst(".novel-cover img, .panel-body img")
        val coverUrl = coverEl?.attr("src")?.toAbsoluteUrlOrNull(domain) ?: manga.coverUrl

        val descEl = doc.selectFirst(".novel-description, .description")
        val description = descEl?.text()?.trim() ?: manga.description

        val tagsEl = doc.select(".tags a, .genres a")
        ...
        val chapterItems = doc.select(".chapter-list a, .chapters a")
        var chapterNumber = 1f
        for (item in chapterItems) {
            val chapterUrl = item.attr("href")
            val chapterTitle = item.text().trim()
