...
@ContentSourceParser("HENTAIFREAK", "HentaiFreak", "en", type = ContentType.HENTAI_VIDEO)
internal class HentaiFreak(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.HENTAIFREAK, pageSize = 24) {

    override val configKeyDomain = ConfigKey.Domain("hentaifreak.org")
    ...
    override suspend fun getListPage(page: Int, order: SortOrder, filter: ContentListFilter): List<Content> {
        ...
        val cards = doc.select("article, .post, .hentry, .video-item")
        for (card in cards) {
