...
@ContentSourceParser("KISSHENTAIZ", "KissHentaiz", "en", type = ContentType.HENTAI_VIDEO)
internal class KissHentaiz(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.KISSHENTAIZ, pageSize = 24) {

    override val configKeyDomain = ConfigKey.Domain("kisshentaiz.com")
    ...
    override suspend fun getListPage(page: Int, order: SortOrder, filter: ContentListFilter): List<Content> {
        ...
        val cards = doc.select(".video-thumb, .video-card, .thumb-item, article")
        for (card in cards) {
