...
@ContentSourceParser("NOVELFULL", "NovelFull", "en", type = ContentType.NOVEL)
internal class NovelFull(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.NOVELFULL, pageSize = 50) {

    override val configKeyDomain = ConfigKey.Domain("novelfull.net")
    ...
    private fun buildFilterTags(): Set<ContentTag> {
        ...
        tags += ContentTag("Latest Release", "latest-release-novel", source)
        tags += ContentTag("Hot Novel", "hot-novel", source)
        tags += ContentTag("Completed", "completed-novel", source)
        tags += ContentTag("Most Popular", "most-popular", source)
        return tags
    }

    override suspend fun getListPage(page: Int, order: SortOrder, filter: ContentListFilter): List<Content> {
        val query = filter.query.orEmpty()
        val tag = filter.tags.firstOrNull()?.key ?: "latest-release-novel"

        val url = if (query.isNotBlank()) {
            "https://$domain/search?keyword=${query.urlEncoded()}&page=$page"
        } else {
            "https://$domain/$tag?page=$page"
        }
    ...
    private fun parseList(doc: Document): List<Content> {
        ...
        val items = doc.select(".list-novel .row, .list-truyen .row")
        for (item in items) {
            val link = item.selectFirst("h3 a, .novel-title a") ?: continue
            ...
            val authorEl = item.selectFirst(".author")
    ...
    override suspend fun getDetails(manga: Content): Content {
        ...
        val title = doc.selectFirst("h3.title")?.text()?.trim() ?: manga.title

        val coverEl = doc.selectFirst(".book img")
        val coverUrl = coverEl?.attr("src")?.toAbsoluteUrlOrNull(domain) ?: manga.coverUrl

        val authorEl = doc.selectFirst(".info a[href*=\"author\"]")
        val author = authorEl?.text()?.trim() ?: ""

        val descEl = doc.selectFirst(".desc-text")
        val description = descEl?.text()?.trim() ?: manga.description

        val tagsEl = doc.select(".info a[href*=\"genre\"]")
        ...
        val chapterItems = doc.select("ul.list-chapter li, #list-chapter a")
    ...
    override suspend fun getChapterContent(chapter: ContentChapter): NovelChapterContent? {
        ...
        return try {
            val doc = webClient.httpGet(url, getRequestHeaders()).parseHtml()
            val contentEl = doc.selectFirst("#chapter-content") ?: doc.selectFirst(".chapter-content")
                ?: doc.selectFirst(".txt") ?: doc.selectFirst(".content")
                ?: doc.selectFirst(".entry-content") ?: doc.selectFirst("#content")
                ?: doc.selectFirst("body")
            val title = chapter.title ?: doc.selectFirst("h2")?.text()?.trim() ?: ""
