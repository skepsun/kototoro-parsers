...
@ContentSourceParser("LNORI", "Lnori", "en", type = ContentType.NOVEL)
internal class Lnori(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.valueOf("LNORI"), pageSize = 50) {

    override val configKeyDomain = ConfigKey.Domain("lnori.com")
    ...
    private fun buildListUrl(page: Int, order: SortOrder, filter: ContentListFilter): String {
        ...
        return "https://$domain/browse?sort=$sort&page=$page$tagParam"
    }

    private fun parseList(doc: Document): List<Content> {
        val items = ArrayList<Content>()
        val seen = LinkedHashSet<String>()
        for (el in doc.select(".novel-item, .post-item, article")) {
            ...
            val title = el.selectFirst("h2, h3, .novel-title, .post-title")?.text()?.trim()
