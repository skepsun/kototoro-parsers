...
@ContentSourceParser("RANOVEL", "Ranovel", "en", type = ContentType.NOVEL)
internal class Ranovel(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.RANOVEL, pageSize = 50) {

    override val configKeyDomain = ConfigKey.Domain("ranovel.com")
    ...
    private fun parseList(doc: Document): List<Content> {
        ...
        val items = doc.select(".novel-item, .card, .list-item")
        for (item in items) {
            ...
            val authorEl = item.selectFirst(".author, .writer")
    ...
    override suspend fun getDetails(manga: Content): Content {
        ...
        val coverEl = doc.selectFirst(".novel-cover img, .cover img")
        val coverUrl = coverEl?.attr("src")?.toAbsoluteUrlOrNull(domain) ?: manga.coverUrl

        val authorEl = doc.selectFirst(".author a, .writer")
        val author = authorEl?.text()?.trim() ?: ""

        val descEl = doc.selectFirst(".novel-summary, .description, .synopsis")
        val description = descEl?.text()?.trim() ?: manga.description

        val tagsEl = doc.select(".genres a, .tags a")
        ...
        val chapterItems = doc.select(".chapter-list li, .chapters a")
