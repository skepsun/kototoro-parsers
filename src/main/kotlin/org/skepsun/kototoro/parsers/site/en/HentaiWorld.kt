...
@ContentSourceParser("HENTAIWORLD", "HentaiWorld", "en", type = ContentType.HENTAI_VIDEO)
internal class HentaiWorld(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.HENTAIWORLD, pageSize = 24) {

    override val configKeyDomain = ConfigKey.Domain("hentaiworld.tv")
    ...
    override suspend fun getDetails(manga: Content): Content {
        ...
        val title = doc.selectFirst("meta[property=og:title]")?.attr("content")
            ?.removeSuffix(" - HentaiWorld")?.trim()
            ?: doc.selectFirst(".entry-title, h1, .post-title")?.text()?.trim()
            ?: manga.title
        ...
        val cover = doc.selectFirst("meta[property=og:image]")?.attr("content")?.toAbsoluteUrlOrNull(domain)
            ?: doc.selectFirst(".wp-post-image, .post-thumbnail img, .entry-thumb img")?.let {
        ...
        val episodeLinks = doc.select("a[href*=/episode/], a[href*=/hentai/]").filter {
    ...
    private fun parseList(doc: Document): List<Content> {
        ...
        val cards = doc.select("article a[href*=/hentai/], .hentai-item a[href*=/hentai/]").filter { el ->
            el.selectFirst("img[alt]") != null || el.selectFirst(".title, .entry-title") != null
        }
        for (link in cards) {
            ...
            val title = link.selectFirst("img[alt]")?.attr("alt")?.trim()
                ?: link.selectFirst(".title, .entry-title, .post-title")?.text()?.trim()
