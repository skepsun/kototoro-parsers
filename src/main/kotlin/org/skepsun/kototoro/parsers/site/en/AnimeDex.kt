...
@ContentSourceParser("ANIMEDEX", "AnimeDex", "en", type = ContentType.VIDEO)
internal class AnimeDex(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.ANIMEDEX, pageSize = 24) {

    override val configKeyDomain = ConfigKey.Domain("animedex.fun")
    ...
    override suspend fun getDetails(manga: Content): Content {
        ...
        val chapterLinks = doc.select("a[href*=/watch/], a[href*=/episode/]")
    ...
    private fun buildListUrl(page: Int, order: SortOrder, filter: ContentListFilter): String {
        ...
        if (q.isNotEmpty()) {
            return "https://$domain/search?query=$q&page=$page"
        }
    ...
    private fun parseList(doc: Document): List<Content> {
        ...
        val cards = doc.select(".AnimeCard_cardWrapper a[href*=/anime/]")
        for (link in cards) {
