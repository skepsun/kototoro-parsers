...
@ContentSourceParser("HAHOMOE", "haho.moe", "en", type = ContentType.HENTAI_VIDEO)
internal class HahoMoe(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.HAHOMOE, pageSize = 24) {

    override val configKeyDomain = ConfigKey.Domain("haho.moe")
    ...
    private fun buildListUrl(page: Int, order: SortOrder, filter: ContentListFilter): String {
        return if (!filter.query.isNullOrBlank()) {
            ...
            "https://$domain/anime/?page=$page$genreParam"
        }
    }
    ...
    private fun parseList(doc: Document): List<Content> {
        ...
        val cards = doc.select("a[href*=/anime/]").filter { el ->
            el.selectFirst("img[alt]") != null || el.selectFirst(".title, .thumb-title") != null
        }
        for (link in cards) {
            ...
            val title = link.selectFirst("img[alt]")?.attr("alt")?.trim()
                ?: link.selectFirst(".title, .thumb-title, .name")?.text()?.trim()
