...
@ContentSourceParser("ANIMEKIZZ", "AnimeKizz", "en", type = ContentType.VIDEO)
internal class AnimeKizz(context: ContentLoaderContext) :
    PagedContentParser(context, ContentParserSource.ANIMEKIZZ, pageSize = 24) {

    override val configKeyDomain = ConfigKey.Domain("animekizz.live")
    ...
    override suspend fun getPages(chapter: ContentChapter): List<ContentPage> {
        ...
        val videoSrc = doc.selectFirst("video source[src]")?.attr("src")
            ...
            ?: doc.selectFirst("[data-video-src]")?.attr("data-video-src")
            ?: doc.selectFirst("[data-player-url]")?.attr("data-player-url")
        if (videoSrc != null) {
            return listOf(ContentPage(
                id = generateUid(chapterUrl),
